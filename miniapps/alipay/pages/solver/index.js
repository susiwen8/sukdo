...
function getChosenImagePath(result) {
  return result?.apFilePaths?.[0] ?? result?.tempFilePaths?.[0] ?? result?.filePaths?.[0] ?? '';
}
...
Page({
  ...
  onLoad(query) {
    const queryPuzzle = query?.puzzle ? decodeURIComponent(query.puzzle) : '';
  ...
  async getCanvasNode() {
    ...
    return new Promise((resolve, reject) => {
      ...
      const selection = query.select(CANVAS_SELECTOR);
  ...
  async extractImageData(filePath) {
    ...
    const imageInfo = await callMy(my.getImageInfo, { src: filePath });
  ...
  async handleUploadImage() {
    try {
      ...
      const chooseResult = await callMy(my.chooseImage, {
        count: 1,
      ...
      if (error && /cancel/i.test(String(error.errorMessage || error.errMsg || error.message || ''))) {
        return;
      }
