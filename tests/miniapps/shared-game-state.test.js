...
const {
  ...
  toggleNotesMode,
  togglePrefillMode
} = require('../../miniapps/shared/game-state.js');
...
test('shared createGameState seeds the current board from the initial puzzle', () => {
...
test('shared selectCell moves the active cursor', () => {
  const state = createGameState({
    puzzle: samplePuzzle,
    solution: sampleSolution
  });
...
test('shared enterDigit fills an editable cell when notes mode is off', () => {
...
test('shared enterDigit toggles notes instead of filling the board when notes mode is on', () => {
...
test('shared clearCell empties editable values and notes', () => {
  let state = createGameState({
    puzzle: samplePuzzle,
    solution: sampleSolution
  });
...
test('shared enterDigit marks the game complete when the final correct value is entered', () => {
  const state = selectCell(
    createGameState({
      puzzle: samplePuzzle,
      solution: sampleSolution
...
test('shared resetGame restores the original puzzle state', () => {
  let state = createGameState({
    puzzle: samplePuzzle,
    solution: sampleSolution
  });
...
test('shared moveSelection keeps the cursor inside the board', () => {
  const state = createGameState({
    puzzle: samplePuzzle,
    solution: sampleSolution
  });
...
test('shared applyHint fills the selected empty cell with the solution value', () => {
  const state = selectCell(
    createGameState({
      puzzle: samplePuzzle,
      solution: sampleSolution
...
test('shared tickClock advances elapsedSeconds until the puzzle is complete', () => {
  ...
  const finished = tickClock(enterDigit(selectCell(running, 0, 2), 4), 5);

  assert.equal(running.elapsedSeconds, 3);
  assert.equal(finished.elapsedSeconds, 3);
});

test('shared createSnapshot and restoreGameState round-trip a playable session', () => {
  let state = createGameState({
    difficulty: 'hard',
    puzzle: samplePuzzle,
    solution: sampleSolution
...
test('shared prefill mode places a tentative digit without deleting existing notes', () => {
  let state = createGameState({
    puzzle: samplePuzzle,
    solution: sampleSolution
  });
...
test('shared clearCell removes a tentative fill but restores the old notes', () => {
  let state = createGameState({
    puzzle: samplePuzzle,
    solution: sampleSolution
  });
...
test('shared locateFirstWrongPrefill jumps to the earliest incorrect tentative cell', () => {
  const tracingPuzzle = [
    [5, 3, 0, 6, 7, 8, 9, 1, 2],
    [6, 0, 2, 1, 9, 5, 3, 4, 8],
    [0, 9, 8, 3, 4, 2, 5, 6, 7],
